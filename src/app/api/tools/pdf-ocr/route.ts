...
import { performOcr } from "@/services/OCRSpaceService";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 3. Process OCR Text Extraction via OCR.Space
    const ocrResult = await performOcr(fileBuffer, fileName, mimeType);

    return apiSuccessResponse(ocrResult, "Text extracted successfully.");
  } catch (error: any) {
    console.error("[API PDF OCR] Failure:", error);
    ...
    return apiErrorResponse(error.message || "Failed to extract text from document", 500);
  } finally {
    ...
    if (fileKey) {
      try {
        ...
        console.error(`[API PDF OCR] Failed to clean up R2 file: ${fileKey}`, cleanupErr);
      }
    }
  }
}
