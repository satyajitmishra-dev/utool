          ...
          const updatedTickets = tickets.map((t) => {
            if (t.id === ticketId) {
              ...
              const updatedTicket = {
                ...t,
                replies: newReplies,
                status: t.status === "resolved" ? ("open" as const) : t.status, // Reopens ticket if resolved
              };
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-3 items-start">
                  <div className="h-7 w-7 rounded-lg bg-indigo-500/10 text-indigo-400 flex items-center justify-center font-bold text-xs select-none">
                    <User className="h-4 w-4" />
                  </div>
                  <div className="flex-1 space-y-1 bg-card/60 border border-border rounded-2xl p-4">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-foreground">{selectedTicket.name}</span>
                      <span className="text-[9px] text-muted-foreground font-semibold">Creator</span>
                    </div>
                ...
                {(selectedTicket.replies || []).map((reply, idx) => {
                  const avatarColor = reply.isAdmin
                    ? "bg-purple-500/10 text-purple-400 border-purple-500/20"
                    : "bg-indigo-500/10 text-indigo-400 border-indigo-500/20";

                  return (
                    <div key={idx} className="flex gap-3 items-start">
                      <div className={`h-7 w-7 rounded-lg border flex items-center justify-center font-bold text-xs shrink-0 select-none ${avatarColor}`}>
                        {reply.isAdmin ? <Shield className="h-4 w-4" /> : <User className="h-4 w-4" />}
                      </div>
                      <div
                        className={`flex-1 space-y-1 border rounded-2xl p-4 ${
                          reply.isAdmin
                            ? "bg-purple-900/5 border-purple-500/15"
                      ...
                      >
                        ...
                        <p className="text-[8px] text-muted-foreground/60 text-right mt-1 font-semibold">
